...
int main() {
  ...
  h.onMessage([&ref_vel, &lane, &planner](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) {
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          // Fill Car details
          //car ego;
          planner.previous_path_x = previous_path_x;
          planner.previous_path_y = previous_path_y;
          planner.ego.InitVariables(car_x, car_y, car_yaw, car_speed, car_s, car_d, ref_vel);
          // Detect Collision from sensor fusion data
          if(prev_size > 0){
            planner.ego.s = end_path_s;
          }
